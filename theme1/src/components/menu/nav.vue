<template>
  ...
  <div v-else>
    <div
      ...
      <va-app-bar v-if="menuConfig.type == 'app-bar'" :bottom="!menuConfig.posTop">
        <va-button icon="menu" flat :rounded="false" @click="$emit('toggleMenu')" />
        ...
        <div v-if="appConfig.state.value.features.showLogin" class="no-shrink-button">
          <va-button flat :rounded="false" v-if="!loggedIn" @click="goToLogin()">Login</va-button>
          <va-button flat :rounded="false" v-else @click="goToProfile()"><span class="hide-min-md"
              style="margin-right: 5px;">Hey
            </span>{{ betterportal.auth.user!.name }}</va-button>
        </div>

        <va-button flat :rounded="false" class="hide-min-md" to="/App/Version">v{{ version }}</va-button>
        <!-- <va-button-dropdown size="small" flat icon="ellipsis">
          Content
        </va-button-dropdown> -->
        <va-button v-show="visible" icon="expand_less" class="hide-min-md" flat :rounded="false"
          @click="scrollToTop()" />
      </va-app-bar>
      <va-navbar v-else-if="menuConfig.type == 'navbar'" class="mb-2">
        <template #left>
          <va-navbar-item>
            <va-button icon="menu" flat :rounded="false" @click="$emit('toggleMenu')" />
            <!-- <va-button icon="home" flat :rounded="false" to="/" v-if="route.path !== '/'" /> -->
        ...
        <template #right>
          <va-navbar-item>
            ...
            <va-button flat :rounded="false" class="hide-min-md" to="/App/Version">v{{ version }}
            </va-button>
            <va-button v-show="visible" icon="expand_less" class="hide-min-md" flat :rounded="false"
              @click="scrollToTop()" />
          </va-navbar-item>
...
const updateBreadcrumbs = (to: RouteLocationNormalized) => {
  setTimeout(() => {
    ...
    breadcrumbs.value = [];
    if (route.path !== '/Account/Client' && betterportal.auth.isLoggedIn && betterportal.auth.clientId !== null) {
