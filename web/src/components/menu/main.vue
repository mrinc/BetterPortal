  ...
  <div v-else>
    <div
      ...
      <va-app-bar v-if="menuConfig.type == 'app-bar'" :bottom="!menuConfig.posTop" color="primary">
        ...
        <div v-if="appConfig.state.value.features.showLogin">
          <va-button color="white" flat :rounded="false" v-if="!loggedIn" @click="goToLogin()">Login</va-button>
          <va-button color="white" flat :rounded="false" v-else>Hey Mitchell R</va-button>
        </div>

        <va-button color="white" flat :rounded="false" to="/App/Version">v{{version}}</va-button>
        <!-- <va-button-dropdown size="small" flat color="#fff" icon="ellipsis">
          Content
        </va-button-dropdown> -->
        <va-button v-show="visible" icon="expand_less" color="#fff" flat :rounded="false" @click="scrollToTop()" />
      </va-app-bar>
        ...
        <template #right>
          <va-navbar-item>
            <div v-if="appConfig.state.value.features.showLogin">
              ...
              <va-button color="#fff" flat :rounded="false" v-else>Hey Mitchell R</va-button>
            </div>
...
const isUserLoggedInInitially = betterportal.auth.isLoggedIn;
...
const loggedIn = ref(isUserLoggedInInitially);
...
const breadcrumbs = shallowRef<{ label: string; }[]>([]);
...
const updateBreadcrumbs = (to: RouteLocationNormalized) => {
  ...
  if (to.meta.breadcrumbs !== undefined && to.meta.breadcrumbs !== null) {
    breadcrumbs.value = (to.meta.breadcrumbs as any[]).map(x => {
